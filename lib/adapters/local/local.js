...
function find(db, requestDef) {
  ...
  return getIndexes(db).then(function (getIndexesRes) {
    ...
    var queryPlans = planQuery(requestDef, getIndexesRes.indexes);

    return Promise.all(queryPlans.map(function (queryPlan) {
      ...
      return {
        docs: utils.intersection(results.map(function (res) {
          return res.docs;
        }))
      };
    });
