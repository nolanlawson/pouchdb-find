...
function massageSelector(selector) {
  ...
  if (fields.length === 1 && fields[0] === '$and') {
    // simplify $and queries
    selector = utils.mergeObjects(selector.$and);
...
//
// similar to "findMatchingIndex", but try to exactly match
// the user's fields against multiple indexes
//
function findMatchingIndexes(userFields, orderMatters, indexes) {
  ...
  userFields = userFields.slice();

  var res = [];

  mainloop: for (var i = 0; i < userFields.length; i++) {
    for (var j = i + 1; j < userFields.length; j++) {
      var subFields = userFields.slice(i, j);
      var foundIndex = false;
      for (var k = 0; k < indexes.length; k++) {
        var index = indexes[k];
        if (checkIndexMatches(index, true, subFields)) {
          res.push({
            index: index,
            fields: subFields
          });
  ...
  var allFoundFields = [];
  res.forEach(function (def) {
    allFoundFields = allFoundFields.concat(def.fields);
  });
  if (allFoundFields.length !== userFields) {
    return null;
  }
  return res;
}
...
function planQuery(request, indexes) {
  ...
  if (!index) {
    // couldn't find a single matching index. try to use multiple
    // indexes or in-memory filtering
    var matchingIndexes = findMatchingIndexes(userFields, orderMatters, indexes);
    if (matchingIndexes) {
      matchingIndexes.forEach(function (matchingIndex) {
        matchingIndex.queryOpts = getQueryOpts(selector, matchingIndex.index);
      });
