...
// given an array of arrays, return the objects found in all
exports.intersection = function (arr) {
  var firstArr = arr[0];
  var res = [];
  for (var i = 0; i < firstArr.length; i++) {
    var element = firstArr[i];
    var foundInAllOthers = true;
    for (var j = 1; j < arr.length; j++) {
      var otherArr = arr[j];
      var foundInThisArray = false;
      for (var k = 0; k < otherArr.length; k++) {
        var otherElement = otherArr[k];
        if (otherElement._id === element._id) {
          foundInThisArray = true;
          break;
        }
      }
      if (!foundInThisArray) {
        foundInAllOthers = false;
        break;
      }
    }
    if (foundInAllOthers) {
